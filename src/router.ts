...
const host = process.env.ROUTER_HOST?.trim() || "127.0.0.1";
const port = parsePort(process.env.ROUTER_PORT);
const expectedToken = process.env.ROUTER_TOKEN;
...
function handleDisconnect(connection: AgentConnection): void {
  ...
  if (disconnected) console.info(`agent disconnected: ${disconnected.descriptor.agentId}`);
}
...
function handleMessage(connection: AgentConnection, raw: string): void {
  ...
  if (message.type === "register") {
    ...
    console.info(`agent registered: ${message.agent.agentId}`);
    return;
  }
